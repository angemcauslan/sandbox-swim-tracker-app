...
const SwimTracker = () => {
  ...
  const toggleSkillProgress = (studentId, skillId) => {
    setStudents(students.map(student => {
      if (student.id === studentId) {
        const currentProgress = student.progress[skillId] || 'not-started';
        let nextProgress;
        if (currentProgress === 'not-started') nextProgress = 'in-progress';
        else if (currentProgress === 'in-progress') nextProgress = 'achieved';
        else nextProgress = 'not-started';
  ...
  const getProgressColor = (status) => {
    if (status === 'achieved') return 'bg-emerald-500 hover:bg-emerald-600';
    if (status === 'in-progress') return 'bg-amber-400 hover:bg-amber-500';
  ...
  const generatePrompt = (studentId = reportSettings.studentId, wordLength = reportSettings.wordLength, reportType = reportSettings.reportType) => {
    ...
    const achievedSkills = studentSkills.filter(skill => 
      student.progress[skill.id] === 'achieved'
    );
    const inProgressSkills = studentSkills.filter(skill => 
      student.progress[skill.id] === 'in-progress'
    );
    ...
    if (inProgressSkills.length > 0) {
      prompt += `Skills In Progress:\n`;
      inProgressSkills.forEach(skill => {
        prompt += `- ${skill.name}\n`;
      });
      prompt += `\n`;
    }
        ...
        {showAddClassForm && (
          <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                <label className="block text-sm font-medium mb-2 text-slate-700">Duration (minutes)</label>
                <select
                                                        ...
                                                        <div key={category}>
                                                          ...
                                                          <div className="space-y-2">
                                                            {skills.map(skill => (
                                                              <button
                                                                key={skill.id}
                                                                onClick={() => toggleSkillProgress(student.id, skill.id)}
                                                                className={`w-full text-left px-3 py-2.5 rounded-lg text-sm font-medium transition-all shadow-sm hover:shadow ${getProgressColor(student.progress[skill.id] || 'not-started')} text-white`}
                                                              >
                                                                {skill.name}
                                                              </button>
                                                            ))}
